...
- **（2026-07-06 追記）「r」はプレイ中にも「retry」として機能する。** 従来はリザルト画面でのみ有効だったが、途中でキー入力を誤った際にリザルト（判定成立）を待たずにやり直せるよう、`resolvePlayingKey` にも同じ `r`/`R` を追加した。ラウンドの「結果」ではなく「今の試行を破棄してやり直す」という同一の操作なので、ラベルも同じ `keyHint.retry` を再利用する
- **（2026-07-06 追記）「[」はプレイ中の「前の問題へ戻る」に割り当てる。** 誤ってスキップした際に取り消す導線として追加した。`p`（paste）は CLAUDE.md のスコープ外リストに将来実装候補として明記されているため避け、本サブセットで使う予定のないブラケットモーションの記号を使った。Attempt は記録しない（「その問題に到達した」というだけでは判定結果ではないため）
